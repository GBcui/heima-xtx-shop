...
const changeCount = async (ev: InputNumberBoxEvent) => {
...
const onChangeSelectedAll = () => {
  // 全选状态取反
  const _isSelectedAll = !isSelectAll.value
  // 前端数据更新
...
const selectListTotal = computed(() => {
...
const selectedListCount = computed(() => {
  return selectList.value.reduce((pre, cur) => {
...
<template>
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrolltolower">
    ...
    <template v-if="memberStore.profile">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar">
        <text @tap="onChangeSelectedAll" class="all" :class="{ checked: isSelectAll }">全选</text>
        <text class="text">合计:</text>
        <text class="amount">{{ selectListTotal }}</text>
        <view class="button-grounp">
          <view
            @tap="gotoPayment"
            class="button payment-button"
            :class="{ disabled: selectedListCount === 0 }"
          >
