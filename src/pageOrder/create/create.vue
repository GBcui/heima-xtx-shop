...
const submitOrder = async () => {
  ...
  const res = await postMemberOrderAPI({
    addressId: isDefaultAddress.value?.id || '',
  ...
  uni.redirectTo({ url: `/pageOrder/details/details?id=${res.result.id}` })
}
</script>
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    >
      <view class="user"> {{ isDefaultAddress?.receiver }} {{ isDefaultAddress.contact }}</view>
      <view class="address">
        {{ isDefaultAddress?.fullLocation }} {{ isDefaultAddress.address }}</view
      >
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !isDefaultAddress?.id }" @tap="submitOrder">
      提交订单
    </view>
  </view>
</template>
