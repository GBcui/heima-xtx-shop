...
const onOrderPay = async (id: string) => {
  ...
  const or = orderList.value.find((item) => item.id === id)
  or!.orderState = OrderState.DaiFaHuo
}
</script>
<template>
  <scroll-view scroll-y class="orders">
    <view class="card" v-for="item in orderList" :key="item.id">
      ...
      <navigator
        ...
        :key="sku.spuId"
        class="goods"
        :url="`/pageOrder/detail/detail?id=${sku.id}`"
        hover-class="none"
      >
        <view class="cover">
      ...
      <view class="action">
        ...
        <template v-else>
          <navigator
            ...
            :url="`/pageOrder/create/create?orderId=id`"
            hover-class="none"
          >
            再次购买
          </navigator>
